...
class MyCustomDrawer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    List<List> options  = [
      ...
      controller.filterTag,
    ];
    List<String> sectionTitles = ["Colors", "Sizes", "Price"];
        ...
        child: Column(
          ...
          children: [
              ...
              return Wrap(
                ...
                children: options[0].asMap().entries.map((entry) {
                  ...
                  return FilterChip(
                    ...
                    selected: isSelected,
                    onSelected: (selected) {
                      controller.select(0, index , option); // Update selection
                    },
                  );
                }).toList(),
              );
              ...
              return Wrap(
                ...
                children: options[1].asMap().entries.map((entry) {
                  ...
                  return FilterChip(
                    ...
                    selected: isSelected,
                    onSelected: (selected) {
                      controller.select(1, index , option); // Update selection
                    },
                  );
                }).toList(),
              );
            ...
            Obx(() {
              return Wrap(
                spacing: 8.0,
                children: options[2].asMap().entries.map((entry) {
                  int index = entry.key;
                  String option = entry.value;

                  bool isSelected = controller.selList[2].contains(index);
                  ...
                  return FilterChip(
                    ...
                    selected: isSelected,
                    onSelected: (selected) {
                      controller.select(2, index , option); // Update selection
                    },
                  );
                }).toList(),
              );
